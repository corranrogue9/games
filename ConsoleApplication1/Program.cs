namespace ConsoleApplication1
{
    ...
    partial class Program
    {
        ...
        public sealed class LonghonrUndoDisplayer<TPlayer> : IDisplayer<UndoableGame<Longhorn<TPlayer>, LonghornBoard<TPlayer>, LonghornMove, TPlayer>, LonghornBoard<TPlayer>, UndoableMove<LonghornMove>, TPlayer>
        {
            //// TODO we already knew displaying was trash, but an undo displayer can't even be general purpose or compose the existing longhorn displayer; it's *really* bad

            private readonly Func<TPlayer, string> transcriber;

            public LonghonrUndoDisplayer(Func<TPlayer, string> transcriber)
            {
                this.transcriber = transcriber;
            }

            public void DisplayBoard(UndoableGame<Longhorn<TPlayer>, LonghornBoard<TPlayer>, LonghornMove, TPlayer> game)
            {
                Console.WriteLine();
                Console.WriteLine();
            ...
            public void DisplayMoves(UndoableGame<Longhorn<TPlayer>, LonghornBoard<TPlayer>, LonghornMove, TPlayer> game)
            {
            ...
            public string TranscribeMove(UndoableGame<Longhorn<TPlayer>, LonghornBoard<TPlayer>, LonghornMove, TPlayer> game, UndoableMove<LonghornMove> move)
            {
                if (move is UndoableMove<LonghornMove>.Native longhornMove)
                {
                    if (longhornMove.Value is LonghornMove.LocationChoice locationChoice)
                    {
                        return $"place the other player at ({locationChoice.Location.Row}, {locationChoice.Location.Column})";
                    }
                    else if (longhornMove.Value is LonghornMove.LocationMove locationMove)
                    {
                        ...
                        return $"take {CowCount(game.Game, locationMove, game.Board.PlayerLocation)} {locationMove.TakeColor} cows{action} and {resultLocation}";
                    }

                    throw new InvalidOperationException($"invalid move type: {longhornMove.GetType()}");
                }
                else
                {
                    return "undo";
            ...
            public void DisplayOutcome(UndoableGame<Longhorn<TPlayer>, LonghornBoard<TPlayer>, LonghornMove, TPlayer> game)
            {
                Console.WriteLine($"{string.Join(",", game.Outcome.Winners)} won the game!");
            }

            public UndoableMove<LonghornMove> ReadMoveSelection(UndoableGame<Longhorn<TPlayer>, LonghornBoard<TPlayer>, LonghornMove, TPlayer> game)
            {
        ...
        public static class NonundoableStrategy
        {
            public static NonundoableStrategy<TGame, TBoard, TMove, TPlayer> Create<TGame, TBoard, TMove, TPlayer>(IStrategy<TGame, TBoard, TMove, TPlayer> strategy)
                where TGame : IGame<TGame, TBoard, TMove, TPlayer>
            {
                return new NonundoableStrategy<TGame, TBoard, TMove, TPlayer>(strategy);
            }
        }

        public sealed class NonundoableStrategy<TGame, TBoard, TMove, TPlayer> : IStrategy<UndoableGame<TGame, TBoard, TMove, TPlayer>, TBoard, UndoableMove<TMove>, TPlayer>
            where TGame : IGame<TGame, TBoard, TMove, TPlayer>
        {
            private readonly IStrategy<TGame, TBoard, TMove, TPlayer> strategy;

            public NonundoableStrategy(IStrategy<TGame, TBoard, TMove, TPlayer> strategy)
            {
                this.strategy = strategy;
            }

            public UndoableMove<TMove> SelectMove(UndoableGame<TGame, TBoard, TMove, TPlayer> game)
            {
                return new UndoableMove<TMove>.Native(this.strategy.SelectMove(game.Game));
            }
        }

        private static void LonghornHumanVsRandom()
        {
            var displayer = new LonghonrUndoDisplayer<string>(_ => _);
            ////var seed = 581316343;
            var seed = Environment.TickCount;
            var random = new Random(seed);
            var exes = "player1";
            var ohs = "player2";
            var longhorn = new Longhorn<string>(exes, ohs, random);
            var game = new UndoableGame<Longhorn<string>, LonghornBoard<string>, LonghornMove, string>(longhorn);
            var driver = Driver.Create(
                new Dictionary<string, IStrategy<UndoableGame<Longhorn<string>, LonghornBoard<string>, LonghornMove, string>, LonghornBoard<string>, UndoableMove<LonghornMove>, string>>
                {
                    ...
                    { ohs, NonundoableStrategy.Create(new MonteCarloStrategy2<Longhorn<string>, LonghornBoard<string>, LonghornMove, string>(ohs, 100000, StringComparer.OrdinalIgnoreCase, random)) },
